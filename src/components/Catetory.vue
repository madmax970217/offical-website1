    ...
    <div class="contentLi fr">
      <div class="catetorytitle">
        ...
        <div class="breadcrumb fr">
          <a href="">首页</a>&gt;<a href="">中心概况</a>&gt;<a href=""
            >校园新闻</a
          >
        </div>
      </div>
      <div class="group_buying">
        <ul>
          <li v-for="(item, index) in catetoryList" :key="index">
            <p class="tit">
              <span class="point fl"></span
              ><a :href="'/index/catetory/aaa/article/' + item.id">{{
                item.title
              }}</a
              ><span class="date">{{ item.publicTime | normalTime }}</span>
            </p>
      ...
      <div class="splitPage">
        <p>
          共{{ tottleArticleNum }}篇（<font color="red">{{ pageNum }}</font
          >/{{ pageCount }}）
          <a @click="jumpNewPage(-4)" href="javascript:void(0)">首页</a
          >&nbsp;&nbsp;
          <a
            @click="!(pageNum == 1) && jumpNewPage(-3)"
            :class="[pageNum == 1 ? 'disabled-link' : '']"
          ...
          &emsp;<span
            ...
            :key="index"
            @click="jumpNewPage(index)"
            :class="[pageNum == index ? 'cur-page' : '']"
            >{{ index }}</span
          >&emsp;
          <a
            @click="!(pageNum == pageCount) && jumpNewPage(-2)"
            :class="[pageNum == pageCount ? 'disabled-link' : '']"
            href="javascript:void(0)"
            >下一页</a
          >&nbsp;&nbsp;
  ...
  methods: {
    jumpNewPage: function(pageState) {
      /*旧的判定是否还有更前页的方法
				...
				*/
      switch (
        pageState //-4表示首页，-3表示上一页，-2表示下一页，-1表示末页
      ) {
        case -4:
