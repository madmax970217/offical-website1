  ...
  <div id="leftNav">
    <aside class="left_nav fl">
      <div class="nav_title">
        ...
        {{ topCatetoryLink | toUpperCase }}
      </div>
      <ul>
        <li
          :class="{ active: value.subLink == curCatetory }"
          v-for="(value, index) in secCatetory"
          :key="index"
        >
      ...
      for (var index in this.allNavData) {
        ...
        if (this.allNavData[index].havesub) {
          for (var index2 in this.allNavData[index].sub) {
            if (
              this.allNavData[index].sub[index2].subLink == this.curCatetory
            ) {
...
.left_nav .nav_title {
  height: 78px;
  line-height: 88px;
  background: url(/static/img/nav_title.png) no-repeat;
  font-size: 10px;
  color: #9c9c9c;
}
.left_nav .nav_title h3 {
  line-height: 78px;
  font-size: 18px;
  margin: 0 10px 0 10px;
}
.left_nav .nav_title h3 a:hover {
...
.left_nav ul li.active span {
  display: block;
  width: 10px;
  height: 11px;
  background: url(/static/img/nav_arrow.png) no-repeat;
  margin: 20px 15px 0 94px;
}
</style>
